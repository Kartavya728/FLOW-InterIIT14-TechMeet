...
export function Reports() {
  ...
  // Fetch reports when category changes and poll every 5 seconds
  useEffect(() => {
    const fetchReports = async () => {
      ...
      try {
        ...
        // Update reports smoothly - only if there are actual changes
        setReports(prevReports => {
          // Check if reports have changed
          if (JSON.stringify(prevReports) !== JSON.stringify(transformedReports)) {
            console.log('📊 Reports updated:', transformedReports.length, 'batches');
    ...
    // Cleanup interval on unmount or category change
    return () => {
      console.log('🛑 Stopping report polling');
      clearInterval(pollInterval);
    };
  }, [selectedCategory, reports.length]); // Added reports.length to dependency array for loading/error logic
