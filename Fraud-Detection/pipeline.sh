...
run_pretrain() {
    ...
    docker-compose -p $PROJECT_NAME -f "$COMPOSE_FILE" --profile pretrain run --rm pretrain
...
start_pipeline_nodes() {
    ...
    docker-compose -p $PROJECT_NAME -f "$COMPOSE_FILE" up -d detector stats-updater feedback report publisher frontend negative-collector
...
stop_all() {
    ...
    cd "$SCRIPT_DIR"
    docker-compose -p $PROJECT_NAME -f "$COMPOSE_FILE" --profile pretrain --profile init down
...
cmd_restart() {
    ...
    print_step "Stopping all containers..."
    cd "$SCRIPT_DIR"
    docker-compose -p $PROJECT_NAME -f "$COMPOSE_FILE" --profile pretrain --profile init down 2>/dev/null || true
